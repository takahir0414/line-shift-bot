...
function dayButton(iso, session, isEmployee) {
  ...
  return {
    ...
    flex: 1,
    contents: [
      { type: "text", text: weekdayLabel(iso), size: "xs", align: "center", color },
      { type: "text", text: formatMD(iso), size: "xs", align: "center", color },
    ],
      ...
      actions: [
        {
          ...
          mode: "time",
          initial: which === "start" ? "17:00" : "22:00",
          min: "08:00",
          max: "27:00",
        },
      ],
...
function summaryMessage(session) {
  ...
  const rows = dates.map((iso) => {
    ...
    const rightText = isDayOff ? "休み希望" : entry ? `${entry.start} - ${entry.end}` : "未入力";
    const rightColor = isDayOff ? TEXT_GRAY : RED;
    return {
      type: "box",
      layout: "horizontal",
      contents: [
        { type: "text", text: `${weekdayLabel(iso)} ${formatMD(iso)}`, size: "sm", flex: 3 },
